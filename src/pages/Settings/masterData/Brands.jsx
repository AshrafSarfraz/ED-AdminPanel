...
export default function Brands() {
  ...
  const load = async () => {
    setLoading(true);
    const data = await apiFetch("/api/brands/all");
  ...
  const save = async () => {
    setError("");
    if (!form.name) { setError("Brand name is required"); return; }
    setSaving(true);
  ...
  const cancel = () => { setEditing(null); setForm({ name: "" }); setError(""); }; // line 2
    ...
    <div className="max-w-[1500px]">
      ...
      {adding && (
        <div className="bg-gradient-to-br from-[#FFF1DD] via-[#FFF8EF] to-white border border-brand-border rounded-[20px] p-4 md:p-5 shadow-[0_4px_20px_rgba(241,90,33,0.06)] mb-4">
          <h2 className="text-[14px] font-bold text-brand-dark m-0 mb-3">{editing ? "Edit" : "Add"} Brand</h2>
          {error && <p className="text-red-600 text-[11px] mb-2">{error}</p>}
          <div className="mb-3">
            <label className="block text-[11px] font-semibold text-brand-dark mb-1">Brand Name <span className="text-red-500">*</span></label>
            <input
              value={form.name}
      ...
      <div className="bg-gradient-to-br from-[#FFF8EF] via-[#FFF8EF] to-white border border-brand-border rounded-[20px] overflow-hidden shadow-[0_4px_20px_rgba(241,90,33,0.06)]">
        ...
        <div className="flex items-center justify-between flex-wrap gap-3 px-4 md:px-6 pt-4 pb-3">
          <div>
            <h1 className="text-[16px] md:text-[18px] font-bold text-brand-dark m-0">Brands</h1>
            <p className="text-[11px] text-brand-muted mt-[2px]">{filtered.length} brands</p>
          </div>
          {!adding && (
            <button
            ...
            <table className="w-full border-collapse table-fixed">
              <thead className="sticky top-0 z-10">
                <tr className="bg-[#FFEDD5]">
                  <th className="w-[50%] px-4 py-[10px] text-left text-[11px] text-[#7c3a1e] font-bold tracking-wide">Name</th>
                  <th className="w-[25%] px-4 py-[10px] text-left text-[11px] text-[#7c3a1e] font-bold tracking-wide">Status</th>
                  <th className="w-[25%] px-4 py-[10px] text-left text-[11px] text-[#7c3a1e] font-bold tracking-wide">Actions</th>
                </tr>
              </thead>
              <tbody>
                {filtered.map(item => (
        ...
        <div className="flex items-center px-4 md:px-6 py-3 border-t border-brand-border">
          <span className="text-[11px] text-brand-muted">Showing {filtered.length} of {items.length} brands</span>
        </div>
      </div>
    </div>
  );
