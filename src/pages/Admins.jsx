  ...
  return (
    <div className="max-w-[1500px] mx-auto px-2 md:px-[10px]">
      ...
      <div className="flex items-center justify-between mb-5">
        <div>
          <h1 className="text-[20px] md:text-[22px] font-extrabold text-brand-dark m-0">Admin Users</h1>
          <p className="text-[13px] text-brand-muted mt-1">{admins.length} total admins</p>
        </div>
        {me.role !== "user" && (
          <button
            onClick={() => setShowModal(true)}
            className="px-5 py-2 bg-brand-gradient text-white rounded-[8px] text-[14px] font-bold border-none cursor-pointer shadow-[0_4px_12px_rgba(241,90,33,0.3)]"
          >
            + Add Admin
          </button>
        )}
      ...
      <div className="bg-gradient-to-br from-[#FFF8EF] via-[#FFF8EF] to-white border border-brand-border rounded-[20px] overflow-hidden shadow-[0_4px_20px_rgba(241,90,33,0.06)]">
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr className="bg-[#FFEDD5]">
                {["Name", "Email", "Role", "Status", "Joined", me.role !== "user" ? "Action" : null]
                  .filter(Boolean)
                  .map(h => (
                ...
                <tr key={a._id} className="border-b border-[#fdf0ea] hover:bg-[rgba(241,90,33,0.05)] transition-colors">
                  <td className="px-4 py-[14px]">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-[14px] font-semibold text-brand-dark">{a.name}</span>
                      {a._id === me._id && (
                        <span className="bg-gray-100 text-brand-gray text-[11px] px-2 py-[2px] rounded-[10px]">You</span>
                      )}
                    </div>
                  </td>
                  <td className="px-4 py-[14px] text-[13px] text-brand-gray">{a.email}</td>
                  <td className="px-4 py-[14px]">
                    <span className={`px-3 py-[3px] rounded-[20px] text-[12px] font-semibold border ${roleClass(a.role)}`}>
                      {a.role}
                    </span>
                  </td>
                  <td className="px-4 py-[14px]">
        ...
        {/* Footer */}
        <div className="px-4 md:px-6 py-4 border-t border-brand-border">
          <span className="text-[12px] text-brand-muted">Showing {admins.length} admins</span>
        </div>
      </div>
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center z-[1000] px-4">
          <div className="bg-white rounded-[20px] p-8 w-full max-w-[460px] shadow-[0_20px_60px_rgba(0,0,0,0.2)]">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-[18px] font-bold text-brand-dark m-0">Add New Admin</h3>
            ...
            <form onSubmit={handleAdd}>
              ...
              <p className="text-[13px] text-brand-muted mb-5">A random password will be generated and sent to their email.</p>
              ...
              <div className="flex gap-3 justify-end">
                ...
                <button type="submit" disabled={saving}
                  className="px-5 py-2 bg-brand-gradient text-white border-none rounded-[8px] text-[14px] font-bold cursor-pointer disabled:opacity-70 shadow-[0_4px_12px_rgba(241,90,33,0.3)]">
