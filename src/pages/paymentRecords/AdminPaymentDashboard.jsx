...
export default function AdminPaymentDashboard() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      apiFetch("/api/admin/commission-records"),          // real per-order commission — our actual profit
    ]).then(([allReceipts, buyers, days, pendingRec, recovered, commissionRecords]) => {
      ...
      // Commission = asal profit — seedha har order ke real commissionAmount ka sum.
      // Pehle ye "Revenue - Expense + Recovered" se approximate kiya jaata tha, lekin
      // Revenue/Expense alag-alag waqt pe record hote hain (invoice creation vs receipt
      // approval vs supplier release), isliye subtract karne se timing mismatch aa jaata
      // — ab seedha commission-records se lete hain, taake Platform Commission page ke
      // number se hamesha match kare.
      const commission = commissionRecords?.overall?.totalCommission || 0;
  ...
  return (
    <div className="max-w-[1500px]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-3 md:gap-4 mb-3">
        ...
        <StatCard value={fmtAmt(stats.totalCommission)} label="Net Commission"  sub="Our actual profit (3% per order)" icon="trending" onClick={() => navigate("/payments/commission")} />
      </div>
