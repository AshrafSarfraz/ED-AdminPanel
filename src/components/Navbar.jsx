...
export default function Navbar() {
  ...
  const [settingOpen,   setSettingOpen]   = useState(
    ["/biddingSettings", "/commissionSettings", "/deliverySettings"].includes(location.pathname)
  );
  const [appConfigOpen, setAppConfigOpen] = useState(
    ["/banners", "/faqs", "/terms"].includes(location.pathname)
  );
  ...
  const appConfigActive = appConfigItems.some(i => location.pathname === i.path);
      ...
      <nav className="flex-1 px-1 md:px-2 py-2 overflow-y-auto overflow-x-hidden">
        ...
        {["superadmin", "admin"].includes(me.role) && (
          <div>
            <div
              title="App Config"
              onClick={() => setAppConfigOpen(p => !p)}
              className={`flex items-center h-10 rounded-lg mb-1 cursor-pointer transition-all duration-200
                justify-center md:justify-start md:gap-[14px] md:px-4
                ${appConfigActive ? "bg-brand-primary" : "hover:bg-brand-lighter"}`}
            >
              <span className={`text-[15px] shrink-0 ${appConfigActive ? "text-brand-white" : "text-brand-gray"}`}><FaMobile /></span>
              <span className={`hidden md:flex flex-1 items-center justify-between text-[12px] font-medium whitespace-nowrap
                ${appConfigActive ? "text-brand-white" : "text-brand-gray"}`}>
                App Config
                <span className={`text-[10px] mr-1 transition-transform duration-200 ${appConfigOpen ? "rotate-180" : ""}`}>▼</span>
              </span>
            </div>
            {appConfigOpen && (
              <div className="hidden md:block ml-4 border-l-2 border-brand-border pl-2 mb-1">
                {appConfigItems.map(item => <DropdownItem key={item.path} item={item} />)}
              </div>
            )}
          </div>
        )}
